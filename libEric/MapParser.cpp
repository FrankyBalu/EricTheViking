...
void
LibEric::MapParser::ParseTilesets(tinyxml2::XMLElement *tilesetRoot, std::vector<Tileset> *tilesets, std::string path) {
...
void LibEric::MapParser::ParseObjectLayer(tinyxml2::XMLElement *pObjectElement, Map *rmap) {
    ...
    for (tinyxml2::XMLElement *e = pObjectElement->FirstChildElement(); nullptr != e; e = e->NextSiblingElement()) {
        if (std::string("object") == e->Value()) {
            ...
            if (gameObject == nullptr) { ;//PLOGE << "Konnte Gameobject nicht erstellen: ";
            } else {
                int x = 0;
                int y = 0;
                int width = 0;
                ...
                for (tinyxml2::XMLElement *properties = e->FirstChildElement();
                     nullptr != properties; properties = properties->NextSiblingElement()) {
                    if (std::string("properties") == properties->Value()) {
                        for (tinyxml2::XMLElement *property = properties->FirstChildElement();
                             nullptr != property; property = property->NextSiblingElement()) {
